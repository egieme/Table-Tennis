...
const SUPABASE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlsenhlbGNoZm5iYWxnbWFqaml3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY3NTI2ODIsImV4cCI6MjA4MjMyODY4Mn0.mNOT9VPHh5f9xSir8UbCU0Ao09pyv3J4Cwyuav5BJAs";
  ...
  try {
    ...
    if (error) {
      if (
        ...
        console.error(
          "FEHLER: Die 'games' Tabelle existiert nicht in der Datenbank!",
        );
        console.error(
          "Bitte führe die SQL-Befehle aus database_setup.sql in deinem Supabase SQL Editor aus.",
        );
        alert(
          "Datenbankfehler: Die 'games' Tabelle fehlt. Bitte führe database_setup.sql aus.",
        );
      }
      throw error;
    }
...
async function loadGameData(gameId) {
  try {
    ...
    if (data) {
      ...
      players[0].scores = Array.isArray(data.p1_scores)
        ? data.p1_scores
        : data.p1_scores || [0, 0, 0, 0];
      players[1].scores = Array.isArray(data.p2_scores)
        ? data.p2_scores
        : data.p2_scores || [0, 0, 0, 0];
  ...
  .on(
    ...
    (payload) => {
      if (payload.new.id === currentGameId) {
        ...
        if (payload.new.sound_promoted)
          soundFiles.promoted = payload.new.sound_promoted;
        if (payload.new.sound_comeback)
          soundFiles.comeback = payload.new.sound_comeback;
        displaySounds();
        ...
        if (comebackCounts[0] > 0) {
          if (!p1Badge) {
            ...
            p1Badge.querySelector(".comeback-count").textContent =
              comebackCounts[0];
          }
        } else if (p1Badge) {
          p1Badge.remove();
        ...
        if (comebackCounts[1] > 0) {
          if (!p2Badge) {
            ...
            p2Badge.querySelector(".comeback-count").textContent =
              comebackCounts[1];
          }
        } else if (p2Badge) {
          p2Badge.remove();
